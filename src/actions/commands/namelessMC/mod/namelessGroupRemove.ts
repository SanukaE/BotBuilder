...
const command: CommandType = {
  name: 'nameless-group-remove',
  description: 'Remove a NamelessMC group from a NamelessMC user.',
  isGuildOnly: true,
  options: [
    {
      name: 'user',
      description:
        'The user you want to remove the group from. (Must have discord account linked)',
      type: ApplicationCommandOptionType.User,
      required: true,
    },
    {
      name: 'group',
      description: 'The group you want to remove.',
  ...
  async script(_, interaction, debugStream) {
    ...
    if (username === interaction.user.username) {
      ...
      await interaction.editReply(
        "You can't remove a group for yourself. Please get a different staff member to do it for you."
      );
    ...
    const response = await fetch(
      process.env.NAMELESSMC_API_URL +
        `/users/integration_name:discord:${username}/groups/remove`,
      {
        method: 'POST',
        headers: {
    ...
    await interaction.followUp({
      content: 'Group was successfully removed from user!',
    });
