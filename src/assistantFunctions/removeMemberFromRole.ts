...
export const declaration: FunctionDeclaration = {
  ...
  description: "Remove a member from a discord role.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      roleId: {
        type: Type.STRING,
        description: "The ID of the role to remove the member from.",
        example: "1334114656461258784",
      },
      memberId: {
        type: Type.STRING,
        description: "The ID of the member to remove from the role.",
        example: "1234567890123456789",
      },
      reason: {
        type: Type.STRING,
        description: "The reason for removing the member from the role (optional).",
        example: "Demoting user from moderator",
      },
      requireDataFromPrev: {
        type: Type.BOOLEAN,
        description:
    ...
    properties: {
      data: {
        ...
        properties: {
          ...
          removedAt: {
            type: Type.STRING,
            description: "The timestamp when the member was removed from the role.",
          },
        },
        required: [
          "roleId",
...
export const script = async (
  ...
  const functionName = "removeMemberFromRole";
  ...
  try {
    ...
    // Fetch the member to remove from the role
    const memberToRemove = await currentGuild.members.fetch(updatableData.memberId);
    if (!memberToRemove) {
      return {
        functionName,
        success: false,
    ...
    if (!memberToRemove.roles.cache.has(role.id)) {
      return {
        functionName,
        success: false,
        data: "Member does not have this role",
      };
    }

    // Remove the role from the member
    await memberToRemove.roles.remove(role, updatableData.reason);
    ...
    return {
      ...
      data: {
        ...
        memberTag: memberToRemove.user.tag,
        memberDisplayName: memberToRemove.displayName,
